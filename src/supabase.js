...
export const rowToKbCard = r => {
  const steps = Array.isArray(r.steps) ? r.steps : null;
  const doneSections = steps ? steps.filter(s => s?.status === 'done').length : 0;
  const progress = steps ? Math.round((doneSections / 16) * 100) : 0;
  return { id: r.id, title: r.title, body: r.body, region: r.region, country: r.country || r.region, section: r.section || "general", department: r.department || 'country_policies', owner: r.owner, status: r.status, author: r.author, updatedAt: r.updated_at, assignedTo: r.assigned_to || null, updateRequest: r.update_request || "", updateRequestedBy: r.update_requested_by || null, updateRequestedAt: r.update_requested_at || null, steps, progress, doneSections };
};
